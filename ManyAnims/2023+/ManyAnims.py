...
def select_anim_files_dialog(*args):
    ...
    selected = cmds.fileDialog2(fileMode=4, dialogStyle=2, caption=caption, fileFilter=file_filter)
...
def hide_codmaya_progress_window():
    ...
    try:
        ...
        print(f"[ManyAnims] ⚠️ Could not hide CoDMayaTools progress bar: {e}")
...
def load_cast_from_path(anim_path):
    try:
        ...
        cmds.warning("⚠️ castplugin not found. Cannot import CAST files.")
        return
    ...
    for idx, cast_file_path in enumerate(files_to_process, 1):
        ...
        # --- Reset before new animation
        try:
            castplugin.utilityClearAnimation()
            print("[ManyAnims] Reset triggered before loading new CAST animation")
        except Exception as e:
            print(f"[ManyAnims] ⚠️ Reset failed: {e}")
        ...
        try:
            ...
            cmds.warning(f"[ManyAnims] ⚠️ Failed to select joints for {cast_file}: {e}")
            continue
        ...
        try:
            ...
            print(f"[ManyAnims] ⚠️ Failed to read notetracks: {e}")
        ...
        try:
            ...
            # --- Safely clear notetracks using the stable 'xanim' ID, not window object
            try:
                if hasattr(CoDMayaTools, "ClearNotes"):
                    CoDMayaTools.ClearNotes("xanim")
                    print("[ManyAnims] ✅ Cleared notetracks via CoDMayaTools.ClearNotes('xanim').")
                else:
                    print("[ManyAnims] ⚠️ ClearNotes() not found in CoDMayaTools.")
            except Exception as e:
                print(f"[ManyAnims] ⚠️ Failed to clear CoDMayaTools notetracks: {e}")

        except Exception as e:
            print(f"[ManyAnims] ⚠️ Export failed for {cast_file}: {e}")
    ...
    try:
        ...
        print(f"[ManyAnims] ⚠️ Scene reset failed after CAST export: {e}")
